...
{ 
 ...
 #error Color format no supported
  ...
  if(App_Config_Ptr->PixelFormatConv == HW_PFC)
    ...
    /**************************************************************************************/
    Utility_DCache_Coherency_Maintenance((void *)(App_Config_Ptr->activation_buffer), 
                                       AI_ACTIVATION_BUFFER_SIZE + 32 - (AI_ACTIVATION_BUFFER_SIZE%32), 
                                       INVALIDATE);
...
void Network_Postprocess(AppConfig_TypeDef *App_Config_Ptr)
{
  ...
  App_Config_Ptr->nn_top1_output_class_proba=*((float*)(App_Config_Ptr->nn_output_buffer)+0);
}
