...
def pose_apply_change_rate(images, boxes, keypoints, images_augmented, boxes_augmented, keypoints_augmented, change_rate=1.0): 
    """
    This function outputs a mix of augmented images and original
    images. The argument `change_rate` is a float in the interval 
    [0.0, 1.0] representing the number of changed images versus 
    ...
    if change_rate == 1.0:
        return images_augmented, boxes_augmented, keypoints_augmented
    ...
    # Create a mask to apply to the images
    mask = change_mask[:,None,None,None]
    mask_not = tf.math.logical_not(mask)
    images_mix = tf.cast(mask_not, images.dtype) * images + tf.cast(mask, images.dtype) * images_augmented

    # Create a mask to apply to the boxes
    mask = change_mask[:,None,None]
    mask_not = tf.math.logical_not(mask)
    boxes_mix = tf.cast(mask_not, tf.float32) * boxes + tf.cast(mask, tf.float32) * boxes_augmented

    # Create a mask to apply to the keypoints
    keypoints_mix = tf.cast(mask_not, tf.float32) * keypoints + tf.cast(mask, tf.float32) * keypoints_augmented

    return images_mix, boxes_mix, keypoints_mix
