...
def generate_LUTs_header_file(path,
    ...
    with open(os.path.join(path, "user_mel_tables.h"), "wt") as f:
        ...
        if floatingPoint =="default" :
            ...
            f.write('extern const float32_t user_melFiltersLut[{}];\n'.format(len(melFilterLut)))
        else:
            f.write('#include "preproc_dpu.h"\n')
            f.write('extern const PREPROC_FLOAT_T user_win[{}];\n'.format(len(hannWin)))
            f.write('extern const PREPROC_FLOAT_T user_melFiltersLut[{}];\n'.format(len(melFilterLut)))
    ...
    with open(os.path.join(path, "user_mel_tables.c"), "wt") as f:
        ...
        if floatingPoint =="default" :
            f.write('const float32_t user_win[{}] = {};\n'.format(
                len(hannWin), hannWin_str))
            f.write('const float32_t user_melFiltersLut[{}] = {};\n'.format(
                len(melFilterLut), melFilterLut_str))
        else:
            f.write('const PREPROC_FLOAT_T user_win[{}] = {};\n'.format(
                len(hannWin), hannWin_str))
            f.write('const PREPROC_FLOAT_T user_melFiltersLut[{}] = {};\n'.format(
                len(melFilterLut), melFilterLut_str))
