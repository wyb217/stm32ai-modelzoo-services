...
def get_model(cfg: DictConfig = None) -> tf.keras.Model:
    ...
    aspp_version = cfg.training.model.aspp
        ...
        if backbone_model == "mobilenet" and backbone_version == "v2" or backbone_model == "resnet50" and backbone_version == 'v1':
            ...
            if aspp_version in  ["v1", "v2"]: 
                model = get_deeplab_v3(input_shape=input_shape, backbone=backbone_model, version=backbone_version,
                                   ...
                                   num_classes=num_classes, output_stride=output_stride, aspp_version=aspp_version)
            else: 
                raise ValueError("\nThe only aspp versions are v1 and v2. Please update the config yaml file")
        else:
            raise ValueError("\nThe only backbone supported so far are Mobilenet v2 and ResNet50 v1. Please update the config yaml "
                             "file accordingly.")
