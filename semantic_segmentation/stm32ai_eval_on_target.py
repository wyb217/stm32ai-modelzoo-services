...
def main(cfg: DictConfig) -> None:
    ...
    test_masks_path = os.path.realpath(cfg.general.test_masks_path)
    test_files_path = os.path.realpath(cfg.general.test_files_path)
    ...
    os.system(f'{main_exec_command} ++operation_mode=evaluation ++general.num_threads_tflite=8 ++general.gpu_memory_limit=2 ++evaluation.target={evaluation_target.lower()} ++general.model_path={model_path} ++dataset.test_path={dataset_path} ++dataset.test_masks_path={test_masks_path} ++dataset.test_files_path={test_files_path} ++dataset.quantization_path="" ++dataset.training_path="" ++dataset.validation_path="" ')
